#ifndef _MARCUS_2D_POTENTIAL_HPP
#define _MARCUS_2D_POTENTIAL_HPP
 ...
 *
 *      THIS FUNCTION CALCULATES EIGENVALUES, EIGENVECTORS, FORCE AND DERIVATIVE COUPLINGS FOR A GIVEN POSITION
 *  
 ****************************************************************
 */
...
namespace {
    ...
    vector<double> param_mass { 2000.0, 2000.0 };
    vector<double> param_omega { 5e-4, 0.0 };
    double param_g = 16.0;
    double param_dG0 = -0.002;
    double param_V = 0.001;

    void output_potential_params() {
        ioer::info("# 2D Marcus Potential parameters: ", 
                    " mass  = ", param_mass,
                    " omega = ", param_omega,
                    " g     = ", param_g,
                    " dG0   = ", param_dG0,
    ...
    vector< complex<double> > cal_H(const vector<double>& r) {
        ...
        H[1+1*2] = 0.5 * mx * pow(wx * (x - param_g), 2) + 0.5 * my * pow(wy * y, 2);
        H[0+1*2] = param_V;
...
#endif // _MARCUS_2D_POTENTIAL_HPP
