...
const vector<double> mass { 1000.0, 1000.0};
const double kT = 9.5e-4;
const vector<double> fric_gamma = { 0.0, 0.0 };

const vector<double> sigma_r { 0.5, 0.5 };
const vector<double> sigma_p = { 1.0, 1.0 };
...
bool argparse(int argc, char** argv) 
{
    ...
    desc.add_options()
        ...
        ("A", po::value<double>(&A), "potential parameter A")
        ("B", po::value<double>(&B), "potential parameter B")
        ("W", po::value<double>(&W), "potential parameter W")
...
void afssh_nd_mpi() {
    // potential para
    set_potenial_params( vector<double> { A, B, W } );
        ...
        if (istep % output_step == 0) {
            ...
            for_each(state.begin(), state.end(), 
                    [&recinfo](const state_t& st) { 
                        ...
                        matrixop::hdiag(cal_H(real(r)), evatmp);

                        // population & momentum
                        if (s == 0) {
                            if (p[0].real() >= 0.0) {
                                recinfo["n0t"] += 1.0;
                                recinfo["px0t"] += p[0].real();
                                recinfo["py0t"] += p[1].real();
                            }
                            else {
                                recinfo["n0r"] += 1.0;
                                recinfo["px0r"] += p[0].real();
                                recinfo["py0r"] += p[1].real();
                            }
                        }
                        else {
                            if (p[0].real() >= 0.0) {
                                recinfo["n1t"] += 1.0;
                                recinfo["px1t"] += p[0].real();
                                recinfo["py1t"] += p[1].real();
                            }
                            else {
                                recinfo["n1r"] += 1.0;
                                recinfo["px1r"] += p[0].real();
                                recinfo["py1r"] += p[1].real();
                            }
                        }
    ...
    if (MPIer::master) {
        ...
        ioer::tabout('#', "t", "n0t", "n0r", "n1t", "n1r", "px0t", "py0t", "px0r", "py0r", "px1t", "py1t", "px1r", "py1r", "KE", "PE", "Etot");

        for (int irec = 0; irec < Nrec; ++irec) {
            if (irec < Nrec - 1) {
                ioer::tabout_nonewline("#", irec * output_step * dt);
            }
            else {
            ...
            ioer::tabout(
                    ...
                    record["n0t"][irec] > 0.0 ? (record["px0t"][irec] / record["n0t"][irec]) : 0.0, 
                    record["n0t"][irec] > 0.0 ? (record["py0t"][irec] / record["n0t"][irec]) : 0.0, 

                    record["n0r"][irec] > 0.0 ? (record["px0r"][irec] / record["n0r"][irec]) : 0.0, 
                    record["n0r"][irec] > 0.0 ? (record["py0r"][irec] / record["n0r"][irec]) : 0.0, 

                    record["n1t"][irec] > 0.0 ? (record["px1t"][irec] / record["n1t"][irec]) : 0.0, 
                    record["n1t"][irec] > 0.0 ? (record["py1t"][irec] / record["n1t"][irec]) : 0.0, 

                    record["n1r"][irec] > 0.0 ? (record["px1r"][irec] / record["n1r"][irec]) : 0.0, 
                    record["n1r"][irec] > 0.0 ? (record["py1r"][irec] / record["n1r"][irec]) : 0.0, 
...
int check() {
    ...
    int N = 500;
    vector<double> xarr = linspace(-5.0, 5.0, N);
    vector<double> yarr = linspace(-5.0, 5.0, N);
